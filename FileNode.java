...
public class FileNode {
	...
	public FileNode findChild(String name) {
		...
		Set<String> childrenList = this.children.keySet();
		
		if(!childrenList.contains(name)){
			Collection<FileNode> childrenColl = this.getChildren();
			FileNode[] childrenLst = childrenColl.toArray(new FileNode[childrenColl.size()]);
			for(int i =0; i < childrenLst.length;i++){
				result = childrenLst[i].findChild(name);
				return result;
			}
		}	
		else{
